...
def get_ion_rotation_measures(vis, inputs_dir, ms1):
    ...
    # The Measurement Set defines the station and dipole positions, the phase centre,
    # and the channel frequencies (and reference frequency) for which the LOFAR beam
    # will be evaluated.
    ...
    for i in range(len(indices)):
        ...
        time_offset = 120.0  # offset time at start and end
        ...
        # Get the results:
        result = getRM(use_azel=use_azel, use_mean=use_mean, timerange=[min_time, max_time], \
                       timestep=time_step, stat_names=stations, stat_positions=lofar_stat_pos, \
                       useEMM=True, TIME_OFFSET=time_offset)
        RM = result['RM']
...
def correct_ion_faraday(vis, ionRM, times, indices):
    ...
    if np.all(times[indices] == times_new[indices_new]):
        ...
        # If this ever happened, the script could be modified to recalculate the ionospheric RMs
    ...
    for i in range(len(indices_new)):
        ...
        response = np.broadcast_to(np.array([[np.cos(rad_rotation), np.sin(rad_rotation)], \
